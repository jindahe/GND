...
本文档将论文的核心理论提炼为可操作的知识体系，涵盖：去相位环面码的混合态结构、anyon 分布、熵分解公式、CMI 的相变行为，以及张量网络模拟方法。
...
**Eq. (13)：CMI 的 RBIM 点缺陷自由能表示**

由于 A 是 $O(1)$ 大小，在粗粒化视角下 CMI 化简为：
$$I(A:C|B) = \overline{F}_{\text{def}}(4r) - \overline{F}_{\text{def}}(2r)$$
其中 $\overline{F}_{\text{def}}(x)$ 是在 $x \times x$ RBIM 中心引入一个**点缺陷**所需的无序平均自由能代价。两项分别对应：
- $\overline{F}_{\text{def}}(4r)$：来自 $H(\pi(\mathbf{m}_A), \mathbf{m}_{BC}) - H(\mathbf{m}_{ABC})$
- $\overline{F}_{\text{def}}(2r)$：来自 $H(\pi(\mathbf{m}_A), \mathbf{m}_B) - H(\mathbf{m}_{AB})$

在 RBIM 临界点，$\overline{F}_{\text{def}}(r)$ 存在以幂律 $r^{-\alpha}$ 衰减的次主项，解释了 CMI 幂律标度 Eq. (11) 的来源。Markov 长度 $\xi$ 与 RBIM 关联长度等同，但拟合指数 $\nu \approx 1.8$ 与文献报道的 $\nu_{\text{RBIM}} \approx 1.5$ 存在偏差，可能源于有限尺寸效应。
