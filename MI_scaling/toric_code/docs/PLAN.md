...
## ⚫ P9：r=1..8 全正值 CMI 生产计划（2026-04-15，按当前服务器重排）

**目标：** 对 `p ∈ {0.05, 0.11, 0.15}`、`r=1..8` 的全部 24 个点，给出统计意义上正值的 CMI 估计，用于 Eq.(12) 拟合与图 3(d) 复现。

**执行环境（2026-04-15）：**
- 硬件：`AMD EPYC 9554` ×2，`128` 物理核 / `256` 线程，`2` 个 NUMA 节点
- 线程设置：`OMP_NUM_THREADS=1`、`OPENBLAS_NUM_THREADS=1`、`MKL_NUM_THREADS=1`
- 脚本：`src/scripts/plot_cmi_vs_r_three_p_chunked.py`
- 并行策略：优先使用 `120 workers`，不直接拉满 `256` 逻辑线程，以避免内存带宽与调度开销吞掉收益

### 9.1 重排原则

- **先校准吞吐，再做主生产**：当前机器较旧计划多出 `~33%` 物理核，但 bMPS 收缩更受内存带宽影响，不能假设 `workers` 线性扩展。
- **并行资源优先换 wall time，不先换更大 chi**：`P9` 的主要风险仍是 `r=8` 的截断偏差，而不是 CPU 不足。
- **分 chi 分批执行**：保留现有脚本接口与缓存逻辑，不修改物理定义、几何和后端选择。
- **只对临界点追加资源**：先完成主干，再只对接近 0 的点加样本或升 chi，避免对全部点过度计算。
...
在当前服务器上，不预设 `120/96 = 1.25` 的理想提速；先用短跑实测校准。若 `128 workers` 相比 `120 workers` 的吞吐提升 `< 8%`，则主生产统一用 `120 workers`。

### 9.3 chi 选择依据

- **双向 SVD 压缩无效**：`P8.8` 已实验证实，问题根源是 `chi` 不足，而非压缩方向。
- **随机化 SVD 不适用**：每步保留比例接近 `1`，无明显加速空间。
- **唯一有效路径**：提高 `chi` + 利用 chunk 并行。

结合现有 chi 扫描结果（以 `p=0.15` 为最严苛判据）：
...
**核心风险：** `r=8, p=0.15` 的真实 CMI 可能已降到与截断偏差同量级；即使当前服务器更强，也未必能通过“统计上正值”的判据。因此 `r=8` 必须先过验证闸门，再决定是否进入主生产。

### 9.4 执行阶段

#### 阶段 0：吞吐校准（约 10–15 min）

**目的**：在当前服务器上，用最重点估计 `workers=96/120/128` 的真实吞吐，避免直接套用旧 `96 workers` 经验值。
...
说明：校准阶段的目标是比较 `workers=96/120/128` 的吞吐，而不是高效生产数据；因此故意使用大量短任务（总计 `300` 个 chunk）来避免“任务数少于 workers”导致的假加速盲区。
...
python -u src/scripts/plot_cmi_vs_r_three_p_chunked.py \
  --r-values 8 --max-bond 128 --max-exact-r 3 \
  --workers 96 --samples 100 --chunk 1 \
  --out-prefix CMI_r8_chi128_w96_calib \
  --out-png CMI_r8_chi128_w96_calib.png
```
...
python -u src/scripts/plot_cmi_vs_r_three_p_chunked.py \
  --r-values 8 --max-bond 128 --max-exact-r 3 \
  --workers 120 --samples 100 --chunk 1 \
  --out-prefix CMI_r8_chi128_w120_calib \
  --out-png CMI_r8_chi128_w120_calib.png
```
...
python -u src/scripts/plot_cmi_vs_r_three_p_chunked.py \
  --r-values 8 --max-bond 128 --max-exact-r 3 \
  --workers 128 --samples 100 --chunk 1 \
  --out-prefix CMI_r8_chi128_w128_calib \
  --out-png CMI_r8_chi128_w128_calib.png
```

**决策规则：**
- 若 `128 workers` 相比 `120 workers` 的 `samples/hour` 提升 `< 8%`，主生产使用 `120 workers`
- 若 `128 workers` 有明显收益且失败率不升，则主生产使用 `128 workers`
- 若三组均出现明显失败或抖动，退回 `96 workers`

**执行记录（2026-04-15）：**
- `w=96`：`real = 165.57 s`，有效样本 `298/300`
- `w=120`：`real = 124.74 s`，有效样本 `298/300`
- `w=128`：`real = 127.23 s`，有效样本 `298/300`
- 结论：`120 workers` 相比 `96 workers` 提速约 `+32.7%`；`128 workers` 相比 `120 workers` 反而慢约 `1.9%`
- 决策：后续 `P9` 主生产统一采用 `120 workers`

#### 阶段 1：chi=128 正值闸门验证（约 35–40 min）

**目的**：确认 `r=7,8` 在 `chi=128` 下是否值得进入主生产。

说明：为避免 `chunk=50` 导致“任务数少于 workers、单个 chunk 过长”的问题，此阶段改用 `chunk=5`，总计 `600` 个 chunk。

```bash
OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 \
python -u src/scripts/plot_cmi_vs_r_three_p_chunked.py \
  --r-values 7,8 --max-bond 128 --max-exact-r 3 \
  --workers 120 --samples 500 --chunk 5 \
  --out-prefix CMI_r7r8_chi128_verify \
  --out-png CMI_r7r8_chi128_verify.png
```

**决策规则：**
- `mean - 2*sem > 0`：该点视为“通过”，可进入主生产
- `mean > 0` 但 `mean - 2*sem <= 0`：保留为“可疑正值”，优先追加样本，不先升 chi
- `mean <= 0`：该点暂不进入主生产；若主要出现在 `r=8`，则将目标范围降为 `r=1..7`

#### 阶段 2：主生产批次 A（`r=1..5`, `chi=64`，约 12 min）

```bash
OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 \
python -u src/scripts/plot_cmi_vs_r_three_p_chunked.py \
  --r-values 1..5 --max-bond 64 --max-exact-r 3 \
  --workers 120 --samples 5000 --chunk 250 \
  --out-prefix CMI_r1to5_chi64 \
  --out-png CMI_r1to5_chi64.png
```

#### 阶段 3：主生产批次 B（`r=6`, `chi=96`，约 45–48 min）
...
#### 阶段 4：主生产批次 C（`r=7,8`, `chi=128`，条件执行，约 5.5–6 h）

仅在阶段 1 通过后执行；若仅 `r=7` 通过，则只跑 `r=7`。
...
**触发条件：**
- `mean > 0` 但 `mean - 2*sem <= 0`
- 或 `p=0.11` 的关键点对 Eq.(12) 拟合影响显著

**优先动作：**
1. 先把该点 `N_total` 从 `5000` 补到 `10000`
2. 只有在“均值稳定为正但系统误差仍明显”时，才考虑把 `r=8` 从 `chi=128` 升到 `chi=160/192`

**不建议：** 在 `mean <= 0` 且接近截断误差量级时，直接对 `r=8` 全量升到 `chi=192`

### 9.5 Wall Time 汇总（当前服务器，推荐 `120 workers`）

| 阶段       | 内容              | 预估 wall time |
|-----------|------------------|---------------|
| 阶段 0     | 吞吐校准（`r=8`, `chi=128`, `N=100`, `chunk=1`, `w=96/120/128`） | `10–15 min` |
| 阶段 1     | 正值闸门验证（`r=7,8`, `chi=128`, `N=500`, `chunk=5`） | `35–40 min` |
| 阶段 2     | `r=1..5`, `chi=64` | `~12 min` |
| 阶段 3     | `r=6`, `chi=96` | `45–48 min` |
| 阶段 4     | `r=7,8`, `chi=128` | `5.5–6 h` |
| **合计**   | `r=8` 通过时的总预算 | **`~6.5–7.0 h`** |

### 9.6 风险与对策

| 风险 | 对策 |
|------|------|
| `r=8, p=0.15` 在 `chi=128` 下仍非正值 | 阶段 1 先闸门验证；若失败，则目标缩减为 `r=1..7` |
| `120→128 workers` 几乎无收益 | 阶段 0 先校准；主生产固定在 `120 workers` |
| `p=0.11` 临界点误差条过大 | 阶段 5 只对该点补样本到 `N_total=10000` |
| `chi=64` 对 `r=5` 不足 | 若阶段 2 后出现负值或极大误差条，再对该点定点升到 `chi=96` |
| 长任务中断 | 继续使用 CSV 缓存断点续跑；重启时自动跳过已完成 chunk |

### 9.7 通过标准

- [ ] 阶段 0：确定当前服务器的推荐 `workers`（预期 `120`）
- [ ] 阶段 1：给出 `r=7,8` 在 `chi=128` 下的 `mean / sem / mean-2sem`
- [ ] `r=1..8`（或调整后范围）的全部目标点完成，每点 `≥ 5000` 有效样本
- [ ] 所有点记录 `mean / sem / z=mean/sem / stable标记`
- [ ] 仅用稳定正值点做 Eq.(12) 拟合，并报告 `R^2`
- [ ] 输出最终合并图（log y 轴），并将未通过点单独标记为 `uncertain`

### 9.8 2026-04-15 日终状态

**已完成：**

- [x] 阶段 0：`r=8, chi=128, N=100, chunk=1` 的吞吐校准完成；推荐 `120 workers`
- [x] 阶段 1：`r=7,8, chi=128, N=500` 闸门验证完成；六个点全部 `mean <= 0`
- [x] 阶段 2：`r=1..5, chi=64, N=5000` 主生产完成
- [x] 阶段 3：`r=6, chi=96, N=5000` 主生产完成
- [x] 阶段 5（部分）：对最难点 `(p=0.15, r=6)` 完成 `chi=128/160/192/256` 扫描，并在 `chi=256, N=5000` 下得到稳定正值

**今日结论：**

- `r=7,8` 在当前 `chi=128` 下不进入主生产
- `r=6, p=0.15` 的已验证稳定正值门槛为 `chi=256`
- 当前 `r<=6` 最佳数据已经足够支撑 **Eq.(12) 的定性比较**
- 但若要给出更强的最终结论，仍需把若干 `mean>0` 但 `mean-2sem<=0` 的边界点补成稳定正值

### 9.9 下一步计划（优先扩样本，而非直接继续大 `r`）

#### 目标

先把 `r<=6` 的最终数据集做成“尽可能全部稳定正值”，再更新 Eq.(12) 最终图与结论；**暂不重启 `r=7,8` 主生产**。

#### 优先级 A：先补样本规模

以下点位当前已为正值，但尚未通过 `mean - 2 sem > 0`：

| 点位 | 当前状态 | 建议动作 |
|---|---|---|
| `(p=0.05, r=5)` | 正值边界 | 先补到 `N_total = 10000` |
| `(p=0.05, r=6)` | 正值边界 | 先补到 `N_total = 20000` |
| `(p=0.11, r=6)` | 正值边界 | 先补到 `N_total = 10000` |
| `(p=0.15, r=3)` | 正值边界 | 先补到 `N_total = 10000` |
| `(p=0.15, r=5)` | 正值边界 | 先补到 `N_total = 20000` |

说明：

- 对 `p=0.05, r=6` 与 `p=0.15, r=5`，按当前误差条估计，`N_total = 10000` 可能仍略紧，因此直接按 `20000` 规划更稳妥
- 所有补样本任务继续采用 chunk 缓存续跑，避免重复计算

#### 优先级 B：仅在扩样本后仍不稳时，再升 chi

若优先级 A 完成后仍有边界点未过稳定判据：

1. `(p=0.15, r=5)`：优先从 `chi=64` 升到 `chi=96`，必要时到 `chi=128`
2. `(p=0.05, r=6)` 与 `(p=0.11, r=6)`：先维持 `chi=96`；只有在扩样本后均值回落或误差异常时，再考虑升 `chi`

#### 优先级 C：最终整合输出

当 `r<=6` 的边界点补强完成后，生成：

- [ ] 最佳数据总表（包含 `source chi / N_total / stable-uncertain`）
- [ ] 仅用稳定点的 Eq.(12) 拟合摘要
- [ ] 最终汇报图（若需要，图中单独标记 `uncertain` 点）
- [ ] 面向论文对比的最终结论文本
