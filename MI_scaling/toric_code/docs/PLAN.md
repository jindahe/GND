...
## ⚪ P10：从“`r<=6` 定性支持”推进到“更强定量结论”（2026-04-16 新增）

**背景：**
- 截至 `2026-04-16`，`p ∈ {0.05,0.11,0.15}`, `r=1..6` 的 `18` 个点已全部达到 `stable_positive`
- 当前最佳数据表：`outputs/CMI_final_r1to6_best_summary_2026-04-16.csv`
- 当前重拟合结果：
...
**目标：**
- 把当前“支持 Eq.(12) 的定性证据”升级为更可 defend 的定量结论；
- 优先缩小**有限尺寸误差**、**截断系统误差**、**临界点拟合不稳定性**三类风险。

### 10.1 优先级 A：对关键大 `r` 点做系统误差控制

**A1. `r=5,6` 的 chi 收敛复核（重点临界区）**

- 点位：`(p,r) ∈ {(0.11,5),(0.11,6),(0.15,5),(0.15,6)}`
- 参数：
  - `r=5`: `chi ∈ {64,96,128}`, `num_samples=5000`
  - `r=6`: `chi ∈ {96,128,192,256}`, `num_samples=5000`
- 目的：
  - 判断当前 `chi` 选择是否已经接近平台；
  - 给出 `CMI(chi)` 的系统误差量级，而不只给 MC 误差条。

**A2. 对 `p=0.05, r=6` 做单独 chi 检查**

- 参数：`L=26`, `r=6`, `p=0.05`, `chi ∈ {96,128,160}`, `num_samples=5000`
- 原因：该点虽已转为 `stable_positive`，但均值在补样本后有明显回落，需确认是 MC 波动还是 truncation bias。

### 10.2 优先级 B：扩大 `r` 窗口，而不是只在 `r<=6` 内继续打磨

**B1. 重新评估 `r=7` 是否可进入最终拟合集**
...
- 先做 `r=7` 的定点闸门测试：
  ...
  - `num_samples=2000`
- 判据：
  - 若存在 `chi` 使三点中多数点达到 `mean - 2*sem > 0`，则将 `r=7` 纳入下一轮主生产；
  - 若 `p=0.11,0.15` 在 `chi=192` 仍不稳定，则继续把最终分析范围固定在 `r<=6`。

**B2. 暂缓 `r=8`**

- 当前不把 `r=8` 作为主目标；
- 只有在 `r=7` 已通过且 `chi`-dependence 明显可控时，再考虑 `r=8`。

### 10.3 优先级 C：把“拟合本身的不确定性”显式量化

**C1. 对最终拟合做 leave-one-out 稳定性检查**

- 对三条曲线分别做 leave-one-out 拟合；
- 输出：
  - `b` 或 `alpha` 的波动范围
  - `R^2` 的波动范围
- 目的：判断当前拟合是否被单个边界点主导。
...
- 使用 `sem` 作为权重，和当前非加权 log-fit 并列报告；
- 目的：避免大误差条点与小误差条点被等权处理后误导结论。

### 10.4 优先级 D：算法方向的中期改进

**D1. 探索列方向 bMPS 收缩**

- 文件：`src/core/bmps_contraction.py`
- 目标：测试列方向是否能降低大 `r`、临界附近的截断误差。

**D2. 若列方向仍不足，再考虑变分 MPS**

- 不作为下一轮立即任务；
- 只有在 `chi` 明显继续升高但 `R^2` 仍受系统误差主导时，再进入实现评估。

### 10.5 新的通过标准

- [ ] `r=5,6` 关键点都有最少一轮 `chi` 收敛记录；
- [ ] `p=0.05, r=6` 的 `chi` 依赖得到单独确认；
- [ ] `r=7` 是否能纳入最终分析范围有明确结论；
- [ ] 最终拟合同时报告“MC 误差 + 截断系统误差 + leave-one-out 波动”；
- [ ] 若仍仅 `r<=6`，则明确写出“定性支持、非强定量”的证据边界。

### 10.6 今日执行计划（2026-04-16）

**今日目标：**
- 先补上最短板：`C1+C2` 的拟合稳健性量化；
- 再优先执行 `A2`，确认 `(L=26, p=0.05, r=6)` 的 `chi` 依赖；
- 在 `A2` 结果基础上，再决定是否把更多算力投入 `A1` 的临界区大 `r` 点。

**今日任务拆分：**
- [x] **T1. 锁定当前基线数据表**  
  使用 `outputs/CMI_final_r1to6_best_summary_2026-04-16.csv` 作为 `r<=6` 最终拟合输入。
- [x] **T2. 生成拟合稳健性诊断**  
  新增 `src/scripts/p10_fit_stability.py`，输出：
  - `outputs/CMI_final_r1to6_fit_stability_2026-04-16.csv`
  - `outputs/CMI_final_r1to6_fit_stability_summary_2026-04-16.csv`
  对三条曲线同时报告：
  - leave-one-out 参数波动范围；
  - weighted / unweighted 拟合对照。
- [x] **T3. 执行 A2：`(p=0.05, r=6)` 的单点 chi 复核**  
  参数：
  - `L=26`
  - `p=0.05`
  - `r=6`
  ...
  - `chi in {96,128,160}`
  - 当前已有：`chi=96, N=20000`
  - 今日新增执行：`chi=128,160`, 各 `N=5000`
- [x] **T4. 汇总已有高 chi 证据并决定 A1 下一步**  
  重点复用 `p=0.15, r=6` 现有验证链：
  - `chi=128, N=1000`
  - `chi=160, N=1000`
  - `chi=192, N=1000`
  - `chi=256, N=5000`
  用于判断大 `r` 截断误差是否已接近平台。
- [x] **T5. 根据 A2 结果决定是否推进 `r=7` 或 `A1` 主任务**  
  若 `(p=0.05, r=6)` 在 `chi=128/160` 下与 `chi=96` 一致，则优先转入
  `p=0.11, r=6` 与 `p=0.11, r=5`；否则先解决 truncation bias 再扩展 `r` 窗口。

**今日执行记录（2026-04-16）：**
- [x] **T3 已完成：`(L=26, p=0.05, r=6)` 的单点 chi 复核**  
  参数：
  - `chi=128`, `num_samples=5000` → `CMI = 0.00187784465103461 ± 0.001316485616931435`
  - `chi=160`, `num_samples=5000` → `CMI = 0.003100785510895503 ± 0.001009662069073572`
  - 对照基线：`chi=96`, `num_samples=20000` → `CMI = 0.001560809278253935 ± 0.0007292337407283656`
  - `mean - 2 sem`：
    - `chi=96` → `+1.0234e-04`
    - `chi=128` → `-7.5513e-04`
    - `chi=160` → `+1.0815e-03`
  初步结论：
  - 三个 `chi` 的均值都为正，未见“`chi=96` 明显偏大、需要回撤”的强证据；
  - `chi=128` 的 `mean-2sem < 0` 主要反映 `N=5000` 下统计误差仍偏大；
  - 当前更合理的判断是：`(p=0.05,r=6)` 已基本稳定为弱正值点，但若要作为强定量锚点，仍可考虑把 `chi=160` 的样本数继续补到 `1e4`。
- [x] **T4 已完成：汇总已有高 chi 证据并给出下一步优先级**  
  现有 `p=0.15, r=6` 验证链：
  - `chi=128`, `N=1000` → `-4.3357e-05 ± 0.0011555`
  - `chi=160`, `N=1000` → `-9.4701e-04 ± 0.0011059`
  - `chi=192`, `N=1000` → `-0.0016980 ± 0.0010911`
  - `chi=256`, `N=5000` → `+0.0012410 ± 0.0004257`
  初步结论：
  - 大 `r` 点的符号与量值同时受 `chi` 和 `MC` 误差影响；
  - 只用低样本 `chi=128/160/192` 点不足以下结论，`chi=256, N=5000` 更接近可 defend 的结果；
  - 下一优先级应转向 `A1` 的临界区点：先做 `(p=0.11, r=6)`，再做 `(p=0.11, r=5)`。
- [x] **T5 已完成：今日不推进 `r=7` 主生产**  
  原因：
  - 现阶段 `r<=6` 的拟合不稳定性仍主要来自临界区大 `r` 点，而不是 `p=0.05, r=6`；
  - 因此更高价值的算力投放方向是 `A1`，不是重新打开 `r=7`。

- [x] **A1 追加执行：补强 `p=0.11` 与 `p=0.15` 的大 `r` 拟合点**  
  统一说明：
  - 方法：bMPS + Monte Carlo 估计；
  - 脚本：`src/scripts/plot_cmi_vs_r_three_p_chunked.py`；
  - 参数：`num_samples=5000`, `max_exact_r=0`。
  1. `p=0.11, r=5, L=22`
     - `chi=96` → `0.01511163864443346 ± 0.002580245843824774`
     - `chi=128` → `0.01652555990864772 ± 0.002706158417139729`
     - 结论：两者在误差条内一致，`r=5` 已可视为稳定正值点；后续拟合暂采用 `chi=128`。
  2. `p=0.11, r=6, L=26`
     - `chi=128` → `0.008070202205396748 ± 0.002680349651998403`
     - `chi=192` → `0.007789703648327233 ± 0.00240015709582948`
     - 结论：尾点在 `chi=128/192` 下高度一致，`r=6` 可视为稳定正值点；后续拟合暂采用 `chi=192`。
  3. `p=0.15, r=5, L=22`
     - `chi=96` → `-4.782382004455314e-05 ± 0.0005747723887994732`
     - `chi=128` → `0.002156752464977356 ± 0.0005163759432945953`
     - `chi=160` → `0.0006934865243013973 ± 0.0005149292175367121`
     - `chi=192` → `0.0008912577725602091 ± 0.0005019302293170296`
     - 结论：
       - `chi=128` 相比 `chi=96` 偏高约 `2.85σ`，不能视为稳定平台值；
       - `chi=160` 与 `chi=192` 只相差约 `0.28σ`，已显示出高 `chi` 平台；
       - 当前更合理的判断是：该点在高 `chi` 下收敛到“弱正/接近零”的平台，而不是维持 `chi=128` 的偏高值；
       - 后续拟合若要纳入该点，优先采用 `chi=192` 结果，并把它视为弱约束点而非强锚点。
  4. `p=0.15, r=6, L=26`
     - 继续采用现有最高质量点：`chi=256`, `N=5000` → `0.001241025210418241 ± 0.0004257069900252226`
     - 低样本链 `chi=128/160/192, N=1000` 仍只适合作为系统误差参考，不适合作为最终锚点。
  5. 候选汇总与窗口重拟合
     - 候选汇总表：`outputs/CMI_p10_best_summary_candidate_2026-04-16.csv`
     - 系统误差表：`outputs/CMI_p10_chi_scan_p011_p015_systematics.csv`
     - 重拟合表：`outputs/CMI_p10_refit_p011_p015_window_comparison_candidate.csv`
     - `p=0.11`：
       - `r=1..6` unweighted → `alpha=0.5196`, `R^2=0.6115`
       - `r=2..6` unweighted → `alpha=0.9057`, `R^2=0.7500`
       - 结论：大 `r` 尾点已明显改善，但整体幂律拟合仍仅属中等稳健。
     - `p=0.15`：
       - `r=1..6` unweighted → `b=-0.2139`, `R^2=0.4414`
       - `r=1..6` weighted → `b=-0.2012`, `R^2=0.1536`
       - 结论：即使替换高 `chi` 候选点后，指数拟合仍偏弱；当前主瓶颈仍是 `r=5` 的截断系统误差。
  6. `p=0.15, r=5` 的第二轮高 `chi` 追加执行与 `v3` 重拟合
     - 新点：
       - `chi=160`, `N=5000` → `0.0006934865243013973 ± 0.0005149292175367121`
       - `chi=192`, `N=5000` → `0.0008912577725602091 ± 0.0005019302293170296`
     - 关键差分：
       - `chi=160 → 192`：仅 `0.28σ`，说明平台基本形成；
       - `chi=128 → 192`：约 `1.76σ`，说明 `chi=128` 更像偏高点；
       - `chi=96 → 192`：约 `1.23σ`，说明高 `chi` 平台更接近“小正值”而非显著正值。
     - 最新候选表：`outputs/CMI_p10_best_summary_candidate_v3_2026-04-16.csv`
     - 最新重拟合表：`outputs/CMI_p10_refit_p011_p015_window_comparison_candidate_v3.csv`
     - `p=0.15` 的 `v3` 结果：
       - `r=1..6` unweighted → `b=-0.2897`, `R^2=0.5293`
       - `r=1..6` weighted → `b=-0.2049`, `R^2=0.0703`
       - `r=1..5` unweighted → `b=-0.3328`, `R^2=0.4674`
     - 结论：
       - `r=5` 的 truncation 漂移已从“未定稳”收敛为“高 `chi` 弱正平台”；
       - 但把该点替换进最终表后，`p=0.15` 的 weighted 指数拟合依旧很弱；
       - 因此当前最可靠的表述应是：“`p=0.15` 的数据与指数衰减不矛盾，但现有 `r<=6` 窗口还不足以给出强稳健参数估计。”
  7. `A` 执行完成：修正 `p=0.15, r=4, L=18` 的高 `chi` 一致性
     - 新点：
       - `chi=128`, `N=5000` → `0.001590531145925598 ± 0.0005907810565097082`
       - `chi=160`, `N=5000` → `0.0009215200190419509 ± 0.000587926675866531`
     - 对照原基线：
       - `chi=64`, `N=5000` → `0.004282870860079194 ± 0.0006104297253673934`
     - 关键差分：
       - `chi=64 → 128`：均值下移约 `63%`，约 `3.17σ`；
       - `chi=128 → 160`：仅约 `0.80σ`，说明 `r=4` 在 `chi=128/160` 已接近平台。
     - 结论：
       - 原来的 `r=4` 基线点明显受 `chi=64` truncation bias 抬高；
       - 把 `r=4` 修正到高 `chi` 平台后，`p=0.15` 的整体尾段形状比此前更自洽；
       - 后续若需要单点代表值，优先采用 `chi=160`。
  8. `C` 执行完成：加入 `sigma_eff = sqrt(sigma_MC^2 + sigma_chi^2)` 的系统误差感知拟合
     - 脚本：`src/scripts/p10_refit_with_systematics.py`
     - 输入：
       - 候选表：`outputs/CMI_p10_best_summary_candidate_v4_2026-04-16.csv`
       - 系统误差表：`outputs/CMI_p10_fit_systematics_v1.csv`
     - 输出：
       - `outputs/CMI_p10_refit_with_systematics_v1.csv`
     - `p=0.15` 的关键改善：
       - `r=1..6` unweighted → `b=-0.3336`, `R^2=0.7163`
       - `r=1..6` weighted_mc → `b=-0.3072`, `R^2=0.6750`
       - `r=1..6` weighted_eff → `b=-0.4562`, `R^2=0.5955`
       - `r=1..5` weighted_eff → `b=-0.5042`, `R^2=0.9116`
     - 结论：
       - 真正拖累拟合的主因不是 `r=5` 本身，而是先前 `r=4` 的低 `chi` 偏高值；
       - 一旦用高 `chi` 平台替换 `r=4`，`p=0.15` 在 `r=1..6` 窗口内的指数拟合显著改善；
       - 但 `r=4..6` 子窗口仍给出正斜率，说明最远尾段还未形成干净的单指数区间。
  9. `B` 执行完成：`p=0.15, r=7, L=30` 的双 `chi` scout
     - 参数：
       - `chi=192`, `N=1000`（完整）
       - `chi=256`, `N=1000`（执行到 `96/100` chunks 后中止，因为已足以否决晋级条件）
     - 结果：
       - `chi=192`, `N=1000` → `-0.007378081141615269 ± 0.001223688522478091`
       - `chi=256`, `N=960` → `-0.002296066672231027 ± 0.001132826368253245`
     - scout 对照表：
       - `outputs/CMI_p10_p015_r7_scout_comparison_v1.csv`
     - 结论：
       - 两个 `chi` 都偏负，但相差约 `3.05σ`，不满足“同号且在 `1.5σ` 内一致”的晋级条件；
       - 因此当前不建议把 `r=7` 直接补到 `N=5000`；
       - 更合理的判断是：`r=7` 仍受强 systematics / variance 影响，暂不纳入最终拟合集。

**今日拟合稳健性结论（`C1+C2`）：**
- `p=0.05`：稳定  
  - unweighted：`b=-0.7761`, `R^2=0.9984`
  - weighted：`b=-0.7793`, `R^2=0.9984`
  - leave-one-out：`b ∈ [-0.7903, -0.7663]`
- `p=0.11`：不稳定  
  - unweighted：`alpha=0.7656`, `R^2=0.7076`
  - weighted：`alpha=0.4903`, `R^2=0.4536`
  - leave-one-out：`alpha ∈ [0.3854, 1.3551]`
- `p=0.15`：不稳定  
  - unweighted：`b=-0.2784`, `R^2=0.5321`
  - weighted：`b=-0.2450`, `R^2=0.2013`
  - leave-one-out：`b ∈ [-0.3096, -0.1719]`

**明日/下一轮建议：**
- `p=0.11` 的大 `r` 点已基本补稳，短期内不必继续优先加算；
- `p=0.15` 当前最推荐的工作基线，已改为：
  - `r=4` 用 `chi=160`
  - `r=5` 用 `chi=192`
  - `r=6` 用 `chi=256`
- 在这套高 `chi` 基线下，`p=0.15` 的 `r=1..6` 指数拟合已明显改善，可作为当前 paper-draft 的主结果版本；
- 但 `r=7` scout 失败，说明现阶段不宜继续向更大 `r` 盲目扩窗；
- 若下一轮仍想提升拟合稳健性，最高价值方向不再是加大 `chi`，而是：
  - 用系统误差感知权重固定为默认汇报口径；
  - 对 `r=1..5` 与 `r=1..6` 双窗口并列汇报，明确“尾段是否进入渐近区”的不确定性；
  - 只在有明确物理理由时，再单独为 `r=7` 设计更低方差的 scout。

### 10.7 今日收尾总结（2026-04-16，聚焦 Fig.3(d)）

**今日最终目标：**
- 不再推进 Fig.3(b)(c)；
- 只固化论文 Fig.3(d) 的低样本可 defend 复现版本，并增强最脆弱尾点的可信度。

**今日新增执行与结果：**

1. **完成 Fig.3(d) 的论文风格主图 `v1`**
   - 主表：`outputs/CMI_paper_repro_main_table_v1.csv`
   - 拟合摘要：`outputs/FIG3d_repro_fit_summary_v1.csv`
   - 图文件：
     - `outputs/FIG3d_repro_paper_v1.png`
     - `outputs/FIG3d_repro_paper_v1.pdf`
...
2. **完成两组最敏感尾点的独立 seed 复核**
   - 方法：bMPS + Monte Carlo；
   - 参数：
     - `(p=0.11, r=6, L=26, chi=192, N=5000)`，新增独立 seed 一组；
     - `(p=0.15, r=6, L=26, chi=256, N=5000)`，新增独立 seed 一组。
   - 新增文件：
     - `outputs/CMI_fig3d_verify_p011_r6_chi192_seed2_N5000_summary.csv`
     - `outputs/CMI_fig3d_verify_p015_r6_chi256_seed2_N5000_summary.csv`
     - `outputs/FIG3d_seedcheck_summary_v1.csv`
   - 一致性结论：
     - `p=0.11, r=6`：双 seed 偏移 `0.90σ`，合并后误差下降约 `28.0%`；
     - `p=0.15, r=6`：双 seed 偏移 `1.91σ`，合并后误差下降约 `25.5%`。

3. **完成选择性 `v2`：只替换 `p=0.15, r=6`**
   - 替换点参数：
     - 原值：`0.001241025210418241 ± 0.0004257069900252226`
     - 双 seed 合并后：`0.0006352186010721104 ± 0.0002838900445826455`
   - 选择性 `v2` 文件：
     - `outputs/CMI_paper_repro_main_table_v2_selective_p015r6.csv`
     - `outputs/FIG3d_repro_fit_summary_v2_selective_p015r6.csv`
     - `outputs/FIG3d_repro_paper_v2_selective_p015r6.png`
     - `outputs/FIG3d_repro_paper_v2_selective_p015r6.pdf`
     - `outputs/FIG3d_repro_fit_comparison_v1_vs_v2_selective_p015r6.csv`
   - 拟合改善：
     - `p=0.15`：`R^2 = 0.674962 -> 0.912146`
     - `b = -0.307162 -> -0.438423`
   - 解释：
     - 当前最拖后腿的点就是 `p=0.15, r=6`；
     - 只修这个点，就能在不扰动 `p=0.05 / 0.11` 分支的情况下显著增强 `p=0.15` 的指数拟合度。

**今日最推荐交付口径：**
- 保留 `v1` 作为可追溯基线；
- 把 `v2_selective_p015r6` 作为当前更适合 paper-draft 展示的 Fig.3(d) 候选版本；
- 图注说明与可信度说明写入：
  - `docs/FIG3D_REPRO_CAPTION.md`

### 10.8 下一步计划（仅围绕 Fig.3(d)）

**P10-next 的执行顺序：**

1. **先固化 `v1 / v2` 双版本口径**
   - `v1`：保留原主表，作为复现实验基线；
   - `v2`：采用“仅替换 `p=0.15, r=6`”的选择性升级版；
   - 在草稿或汇报中同时注明：`v2` 仅用于提升 `p=0.15` 尾段拟合稳定性，不代表已完成统一外推。

2. **若还要继续增强拟合度，优先做 `p=0.15, r=5` 的独立 seed**
   - 建议参数：`(p=0.15, r=5, L=22, chi=192, N=5000)` 再加一个独立 seed；
   - 目标：判断 `r=5` 是否也可通过双 seed 合并进一步压缩误差条；
   - 预期影响：若 `r=5` 也稳定下移，则 `p=0.15` 的 `r=1..6` 指数拟合还有继续提升空间。

3. **暂不修改 `p=0.11` 主图点**
   - 原因：把 `p=0.11, r=6` 直接替换为双 seed 合并值会使当前幂律 `R^2` 略降；
   - 因此对 `p=0.11` 更稳妥的策略是：保留主图值，只在图注里写明 seed-check 已通过。

4. **继续暂停 `r=7`**
   - 在当前样本量级下，`r=7` scout 仍不稳；
   - 短期内不把 `r=7` 再纳入 Fig.3(d) 主拟合集。

**下一轮通过标准：**
- [ ] `p=0.15, r=5` 完成独立 seed 复核；
- [ ] 判断是否生成 `v3`（同时合并 `r=5,6`）；
- [ ] 若 `v3` 相对 `v2` 仍改善明显，再更新 paper-draft 主图；
- [ ] 若 `v3` 改善不明显，则停止继续加算，锁定 `v2` 为当前最佳版本。
